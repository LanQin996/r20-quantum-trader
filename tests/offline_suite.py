...
class OfflineGuard:
    ...
    def audit(self, event, args):
        ...
        if event == 'subprocess.Popen':
            command = args[1]
            tokens = (list(command) if isinstance(command, (list, tuple))
                      else [part.strip('"') for part in shlex.split(str(command), posix=False)])
            ...
            allowed = (executable == 'uname' and tokens[1:] == ['-p']) or (
                ...
                executable in ('python3', 'python', Path(sys.executable).stem)
